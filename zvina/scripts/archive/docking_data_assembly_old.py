...
# 					d_d=self.dock_dir, d=dock, l=lig)
...
# 					self.data_dic[pose][atom] = 0
	...
	def write_alldata_csv(self):
		...
		# If it doesn't, write it
			...
			if not self.is_assembled: self.assemble_dic()
			if not self.energies_props_gotten: self.get_lig_energies_properties()
			if not self.is_bs_listed: self.get_binding_sites_list()
			...
			# Get energies, properties, and pvr data
			basic_fieldnames = ['key', 'lig', 'model']
			energies_properties_fieldnames = self.energies_dic.keys() + self.properties_dic.keys()
			pvr_fieldnames = ['E', 'rmsd_lb', 'rmsd_ub', 'pvr_effic',
				'pvr_n_contacts', 'torsdof', 'pdb_address'] # (processed vina results)
			...
			for bs in self.binding_sites_list:
				...
				binding_site_fieldnames.append("{}_aiad".format(bs))
				binding_site_fieldnames.append("{}_icpd".format(bs))
			all_resi_fieldnames = self.prot_resis_list # + self.prot_resis_atoms_list
			fieldnames = basic_fieldnames + energies_properties_fieldnames + binding_site_fieldnames + all_resi_fieldnames
