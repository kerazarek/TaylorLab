...
# Total
densities_energy_by_lig_allsites <- ggplot(data=data, aes(x=E)) +
  geom_density() +
  xlab("Binding energy (kcal/mol)") +
  ggtitle("Overall Density of Dockings versus Binding Energy") +
  theme(legend.title=element_text(face="bold"),
        plot.title=element_text(face="bold"))
densities_energy_by_lig_allsites

# Grid of ligs
densities_energy_by_lig_allsites <- ggplot(data=data, aes(x=E, color=LIG, group=LIG)) +
  geom_density() +
...
# # rm(binding.distrib.plot)
# # binding.distrib.plot$LIG <- ligset.list
...
barplot_bindingdist_by_lig <- melted.analysis.bindingsites <- melt(analysis.bindingsites, id.vars = "bs_cat")
ggplot(data=melted.analysis.bindingsites, aes(x=bs_cat, y=value, fill=variable)) +
...
ggplot(data=dan.data, aes(x=Ligand, y=Percent.Inhibition, fill=Ligand)) +
